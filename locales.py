...
UI = {
    "zh": {
        ...
        "s3_floor_note": {"tw": "本清單只列 <b>60 分以上</b>(回測歷史期望為正);50–59 分摺疊於下方「低信心觀察」,50 分以下不列入(可用上方『查個股』查任一檔)。屬波段參考:回測顯示需持有約 20 個交易日(約一個月),短線(5/10 日)無優勢。",
                          "us": "本清單只列 <b>60 分以上</b>(分數越高、條件越齊);50–59 分摺疊於下方觀察區,50 分以下不列入(可用上方『查個股』查任一檔)。美股尚無回測,門檻為信心排序而非歷史勝率。"},
        "s3_watch_title": "▸ 低信心觀察(50–59 分;歷史期望偏負,點開展開)",
    ...
    "en": {
        ...
        "s3_floor_note": {"tw": "Only scores <b>60+</b> are listed (positive historical expectancy in backtest); 50–59 are folded under “Low-conviction watch” below, and sub-50 are omitted (search any ticker above). These are swing ideas — the backtest shows you need to hold ~20 trading days (about a month); there is no edge over 5/10 days.",
                          "us": "Only scores <b>60+</b> are listed (higher score = more criteria met); 50–59 are folded into the watch area below, sub-50 omitted (search any ticker above). US has no backtest yet, so the threshold reflects conviction ordering, not historical win rate."},
        "s3_watch_title": "▸ Low-conviction watch (scores 50–59; negative historical expectancy — click to expand)",
