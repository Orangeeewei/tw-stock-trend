...
UI = {
    "zh": {
        ...
        "s2_note": "📈 <b>2 年回測:領頭羊整體小幅贏大盤</b>(多頭時 10 日約 +0.9%、20 日 +0.5% 超額)——動能策略確實有效,且整體比補漲(③)更穩。但屬「少數大贏、多數小輸」的右尾,命中率不到一半。⚠️ <b>漲最兇的前幾名反而最易回檔(別追高)</b>。此區用來看哪些產業有真龍頭,順序非進場優先序;高信心逐檔進場仍以補漲候選(③ ≥70 分)為主。",
        ...
        "s3_floor_note": {"tw": "本清單只列 <b>60 分以上</b>(回測歷史期望為正);50–59 分摺疊於下方「低信心觀察」,50 分以下不列入(可用上方『查個股』查任一檔)。屬波段參考:回測顯示需持有約 20 個交易日(約一個月),短線(5/10 日)無優勢。",
                          "us": "⚠️ <b>美股評分與台股不同尺度、且尚未回測</b>(無法人/月營收資料,分數來自價量結構,天生偏高)——此處只顯示<b>相對強度前 8 強</b>,其餘摺疊於下方,並<b>非</b>勝率門檻。美股回測完成後(資料累積中)會改用實證門檻。"},
    ...
    "en": {
        ...
        "s2_note": "📈 <b>2-year backtest: leaders modestly beat the index overall</b> (~+0.9% excess over 10 days, +0.5% over 20 in bull markets) — momentum works, and is steadier as a cohort than the catch-up names (③). But it's a right-tail: a few big winners, most slightly lag, win rate under half. ⚠️ <b>The hottest names tend to revert (don't chase)</b>. Use this to spot which sectors have real leaders; order is not a buy priority. High-conviction entries still come from the catch-up candidates (③, 70+).", "days_unit": "{n}d",
        ...
        "s3_floor_note": {"tw": "Only scores <b>60+</b> are listed (positive historical expectancy in backtest); 50–59 are folded under “Low-conviction watch” below, and sub-50 are omitted (search any ticker above). These are swing ideas — the backtest shows you need to hold ~20 trading days (about a month); there is no edge over 5/10 days.",
                          "us": "⚠️ <b>US scores are on a different scale from TW and not yet backtested</b> (no institutional/revenue data — scores come from price/volume structure and run high by nature). Only the <b>top 8 by relative strength</b> are shown here; the rest are folded below. This is <b>not</b> a win-rate threshold — once the US backtest is ready (data still accumulating) it will use an evidence-based cutoff."},
