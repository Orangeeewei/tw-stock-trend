...
CONFIG = {
    ...
    # 顯示排序(嚴選與平衡共用):外資5日買超>0 優先、再按投信5日買超降冪。
    # 只影響顯示順序,不改入選集合與績效數字。依據 scripts/inst_search.py 方向 D2
    #「投信+外資同步買超」cohort:test 超額 +0.92% vs 純嚴選 +0.69%(投外同步較強)。
    "sort": ["foreign_net5", "trust_net5"],
